...
requirements work with it, along with the agents that do it, embedded in the
binary. Installing them needs no network access and no repository checkout:
...
.. list-table::
   ...
   * - Agent
     - Covers
   * - ``rst-doc-expert``
     - Authoring, organising, and building RST documentation with bartleby.

The two differ only in the shape they install as. A skill becomes
``<dir>/<name>/SKILL.md``; an agent becomes ``<dir>/<name>.md``, which is what
an agent runtime reads out of its agents directory.

``--project`` installs into ``.claude/skills`` or ``.claude/agents`` in the
current directory instead of the home directory, so they travel with a checkout
rather than being installed once per machine:

.. code-block:: bash

    bartleby skills install --project
    bartleby agents install --project
